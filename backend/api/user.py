...
def register():
    ...
    new_user = User(
        username=data.get('nickname','暂无'),  # 使用username而不是nickname
        email=email,
        avatar=data.get('avatar', ''),  # 保留avatar字段
    )
...
@jwt_required()
def edit_profile():
    """Edit the user's profile information such as username, email and avatar.
    ...
    if 'username' in data:  # 使用username而不是nickname
    ...
    """Edit the user's profile information such as username, and email.
